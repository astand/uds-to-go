...
class DidHandler {
 ...
 public:
  ...
  virtual DidResult ReadDID(uint32_t did, uint8_t* data, size_t capacity, size_t& len_out, NRCs_t& nrc_out) = 0;

  // DID not supported: returned DidResult::Ignored
  // DID pos handling:  returned DidResult::Positive
  // DID neg handling:  returned DidResult::Negative, nrc_out is filled
  virtual DidResult WriteDID(uint32_t did, const uint8_t* data, size_t len, NRCs_t& nrc_out) = 0;
};
